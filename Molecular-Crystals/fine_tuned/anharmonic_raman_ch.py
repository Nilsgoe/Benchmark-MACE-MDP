...
files = [
    ...
    if os.path.isfile(os.path.join("cifs", f)) and f.lower().endswith(".in") and f.lower().startswith("as") # normally  f.lower().endswith(".cif")
]
molecules = [{"name": os.path.splitext(f)[0].split("_p")[0], "cif": os.path.join("cifs", f)} for f in sorted(files)]  # [{"name": os.path.splitext(f)[0], "cif": os.path.join("cifs", f)} for f in sorted(files)] 
mae_calc=MACECalculator(model_paths="../ASPIRIN/MACE_model_swa.model",device="cuda", default_dtype="float64")
