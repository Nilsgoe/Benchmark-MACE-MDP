...
def backend_ipi(t, timeunit, a_xx,a_xy,a_xz,a_yy,a_yz,a_zz, mlag, ftpad, ftwin):
    ...
    #Kubo 
    num = beta * HBAR
    denom = 2*np.pi *omega_si**2 *(1+ np.exp(-beta * HBAR * omega_si)) 
    F= num / denom
    #F=1
    ...
    I_aniso = 0.1 * S_aniso
    I_paper = soft_floor(I_iso + (7.0/3.0)*I_aniso)
    kubo = (1-np.exp(-beta * HBAR * omega_si)) / (beta * HBAR * omega_si)
    #kubo=1
    I_paper *= kubo
...
def main():
    ...
    ap.add_argument("--wn_min", type=float, default=0.0)
