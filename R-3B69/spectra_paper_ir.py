...
csv_file = "./mace_off/IR/ir_results_large_SPICE/spectra_matchscores.csv"
output_dir = "./paper_ir_spectra/"
...
# ============================================================
# Function to make a figure for one prefix group
# ============================================================
def make_group_figure(group_prefix: str):
    ...
    for row_idx, compound in enumerate(compounds_group):
        ...
        if mace_data.ndim == 1:
            mace_data = mace_data.reshape(1, -1)
        if mace_data.ndim != 2 or mace_data.shape[1] < 2:
            print(f"⚠️ Bad MACE CSV format for {compound}, skipping...")
            continue

        freq_mace = mace_data[:, 0].astype(float)
        mace_intensity = mace_data[:, 1].astype(float)
        ...
        if inten_dft.size == 0:
            ...
            continue
        inten_dft = inten_dft / np.max(inten_dft)

        # --- Broaden both spectra ---
    ...
    fig_name = os.path.join(output_dir, f"IR_{group_prefix}_three_examples")
    ...
    csv_path = os.path.join(output_dir, f"IR_{group_prefix}_three_examples.csv")
